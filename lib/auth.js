...
export async function handleLogin(body, kvUrl, kvToken) {
  ...
  try {
    ...
    if (role === 'owner') {
      ...
      // scoping the check the same way it always did.
      const salon = salonId
        ? salons.find(s => s.id === salonId)
        : salons.find(s => s.ownerUsername === username && s.ownerPassword === password);
...
export async function handleChangePassword(body, kvUrl, kvToken) {
  ...
  if (type === 'admin_self') {
    ...
    if (!(await verifyAdminPassword(currentPassword, kvUrl, kvToken))) {
  ...
  if (type === 'admin_set') {
    ...
    if (!(await verifyAdminPassword(adminPassword, kvUrl, kvToken))) {
