...
import { getSalonsDb, setSalonsDb, getAdminDb, setAdminDb, checkRateLimit, acquireBillingLock, releaseBillingLock } from './kv.js';
...
function defaultResetPassword(name) {
  ...
  // A fixed "123" suffix (derived from the worker's own name, itself
  // public-facing — shown to every customer picking a barber) meant
  // anyone who knew or guessed the convention could log in as any
  // barber/owner who hadn't yet changed their reset password, with no
  // forced rotation on next login to force the issue. A random 4-digit
  // suffix keeps the same "short enough to read aloud over the phone"
  // property while adding real entropy (10,000 possibilities) an
  // attacker can't shortcut just by knowing the naming pattern.
  const suffix = String(Math.floor(1000 + Math.random() * 9000));
  return (base || 'utente') + suffix;
}
    ...
    if (role === 'owner') {
      ...
      if (username === salon.ownerUsername && verifyPasswordAny(password, salon.ownerPassword)) {
        if (!isHashedPassword(salon.ownerPassword)) {
          // salons_db is one JSON blob shared by the whole platform — every
          // OTHER writer of it (bulk salon save, delete-worker, billing
          // actions...) takes this same per-salon lock specifically to
          // avoid a lost-update race (two unrelated salons' concurrent
          // writes both reading the same pre-write snapshot, the later SET
          // silently discarding the earlier one's change). This
          // opportunistic upgrade used to be the one unlocked writer of
          // that blob. Best-effort: if another writer holds the lock right
          // now, just skip the upgrade this time — the login itself already
          // succeeded against the legacy plaintext value, and the next
          ...
          if (locked) {
            try {
              ...
              if (freshSalon && !isHashedPassword(freshSalon.ownerPassword)) {
                freshSalon.ownerPassword = hashPassword(password);
                await setSalonsDb(kvUrl, kvToken, freshSalons);
              }
            } catch (e) {
              console.error('[LOGIN] owner password upgrade failed:', e.message);
            } finally {
    ...
    if (role === 'barber') {
      ...
      if (w && verifyPasswordAny(password, w.password)) {
        if (!isHashedPassword(w.password)) {
          // See the matching comment in the owner branch above — same
          // shared-blob lost-update hazard, same best-effort-under-lock fix.
          const locked = await acquireBillingLock(kvUrl, kvToken, salon.id);
          if (locked) {
            try {
              const freshSalons = await getSalonsDb(kvUrl, kvToken);
              const freshSalon = freshSalons.find(s => s.id === salon.id);
              const freshWorker = freshSalon && (freshSalon.workers || []).find(x => x.id === w.id);
              if (freshWorker && !isHashedPassword(freshWorker.password)) {
                freshWorker.password = hashPassword(password);
                await setSalonsDb(kvUrl, kvToken, freshSalons);
              }
            } catch (e) {
              console.error('[LOGIN] barber password upgrade failed:', e.message);
            } finally {
              await releaseBillingLock(kvUrl, kvToken, salon.id);
            }
          }
...
export async function handleChangePassword(body, kvUrl, kvToken, req) {
  ...
  if (type === 'self') {
    ...
    // Same shared-blob lost-update hazard as the login-time opportunistic
    // upgrade in handleLogin above — every other salons_db writer in the
    // codebase takes this per-salon lock; this endpoint used to be one of
    // the ones that didn't. Unlike that best-effort upgrade, changing a
    // password is the entire point of this request, so a failure to
    // acquire the lock is reported to the caller instead of silently
    // skipped.
    const locked = await acquireBillingLock(kvUrl, kvToken, salonId);
    if (!locked) return { status: 503, json: { success: false, error: 'service_unavailable' } };
    try {
      const salons = await getSalonsDb(kvUrl, kvToken);
      const salon = salons.find(s => s.id === salonId);
  ...
  if (type === 'admin_self') {
    ...
    // Every other admin_db writer (the login-time opportunistic upgrade
    // above, handleUpdateHomepagePhotos in api/sync.js) spreads the
    // existing record first. This one used to construct a brand-new
    // object containing only {username, password}, silently wiping
    // homepagePhotos/homepageAd and the cached PayPal product/plan ids
    // every time an admin changed their own credentials.
    const admin = await getAdminDb(kvUrl, kvToken);
    await setAdminDb(kvUrl, kvToken, { ...admin, username: newUsername, password: hashPassword(newPassword) });
