...
export async function acquireBarberDayLock(kvUrl, kvToken, salonId, workerId, dateISO) {
  ...
  // Short retry loop, not a queue — booking requests are quick (one overlap
  // check + one SET NX), so a genuine collision resolves within a few
  // hundred ms. Giving up after that surfaces as a normal "slot taken"
  // conflict to the second caller instead of hanging the request.
  for (let attempt = 0; attempt < 10; attempt++) {
    const res = await kvCmd(kvUrl, kvToken, ['SET', key, '1', 'NX', 'EX', '5']);
