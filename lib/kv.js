...
export async function acquireBarberDayLock(kvUrl, kvToken, salonId, workerId, dateISO) {
  ...
  // for the full 4s.
  // This lock's own critical section (in api/sync.js) re-fetches the
  // ENTIRE bookings hash via getAllBookings before running the overlap
  // check — api/daily-health-check.js already flags >8000 booking rows as
  // a latency concern with no pagination/archiving. If that read ever grew
  // slow enough to approach this lock's own 10s TTL, the lock could expire
  // mid-critical-section and let a second request in, reopening the exact
  // overlap-check TOCTOU window this lock exists to close. Not reachable
  // at today's table size — noted here so it's found alongside that
  // existing warning if the table ever grows enough to matter.
...
// Guards a single booking's read-modify-write in api/sync.js's existing-
// booking UPDATE branch (status changes — "Fatto"/"Annulla") — that branch
// took no lock at all, so two staff devices flipping the SAME booking's
// status within the same few seconds (a front-desk tablet and a barber's
// phone both on "Oggi" is completely ordinary) raced on an unconditional
// HSET with no version check: whichever write physically landed last in
// KV won, non-deterministically, on a revenue-relevant field
// (completed vs cancelled) — and the LOSING request's own response still
// echoed ITS OWN change as if it had succeeded (built from that request's
// own in-memory merge, not a fresh re-read), showing that device a false
// "salvato" confirmation for an action that was actually discarded.
function bookingLockKeyFor(bookingId) {
  return `booklock:${bookingId}`;
}
export async function acquireBookingLock(kvUrl, kvToken, bookingId) {
  const key = bookingLockKeyFor(bookingId);
  const maxWaitMs = 3000;
  const start = Date.now();
  let delay = 30;
  while (Date.now() - start < maxWaitMs) {
    const res = await kvCmd(kvUrl, kvToken, ['SET', key, '1', 'NX', 'EX', '8']);
    if (res === 'OK') return true;
    await new Promise(r => setTimeout(r, delay + Math.floor(Math.random() * delay)));
...
export async function releaseBookingLock(kvUrl, kvToken, bookingId) {
  try { await kvCmd(kvUrl, kvToken, ['DEL', bookingLockKeyFor(bookingId)]); } catch { /* best-effort */ }
}

// Guards a single salon's read-modify-write in api/paypal-webhook.js —
// PayPal can deliver events for different salons within milliseconds of
...
// ttlSeconds defaults to 10 (plenty for the pure-KV critical sections most
// callers use) but handleCreateBillingCheckoutSession in api/sync.js holds
// this lock across multiple PayPal network calls (cancel a stale
// subscription, lazily create the shared Plan, create the new Subscription)
// that can plausibly exceed 10s on a slow response/cold start — if the key
// expired mid-request, a second concurrent request could acquire the "lock"
// and race it, creating two live subscriptions for one salon. Callers doing
// real network I/O inside the critical section should pass a longer value.
export async function acquireBillingLock(kvUrl, kvToken, salonId, ttlSeconds = 10) {
  const key = billingLockKeyFor(salonId);
  const maxWaitMs = 4000;
  const start = Date.now();
  let delay = 40;
  while (Date.now() - start < maxWaitMs) {
    const res = await kvCmd(kvUrl, kvToken, ['SET', key, '1', 'NX', 'EX', String(ttlSeconds)]);
    if (res === 'OK') return true;
    await new Promise(r => setTimeout(r, delay + Math.floor(Math.random() * delay)));
    delay = Math.min(delay * 1.5, 300);
  }
...
// Guards the "push_subscriptions" blob's read-modify-write — api/subscribe.js
// (register/re-register a device), api/send-reminders.js (prune dead
// endpoints after a failed push) all fetch the whole array, mutate it in
// memory, and SET it back, with no lock. Two of these landing close together
// (a customer subscribing right as the reminder cron prunes a stale entry,
// say) could otherwise both read the same pre-write snapshot and the later
// SET silently discards the earlier writer's change — a legitimate new
// subscription disappearing, or a dead one un-pruned. Same short-retry shape
// as the other locks here; a caller that can't get in within the budget
// should just skip the write rather than risk clobbering, since none of
// these are on a user-blocking critical path.
const PUSH_SUBS_LOCK_KEY = 'lock:push_subscriptions';
export async function acquirePushSubsLock(kvUrl, kvToken) {
  const maxWaitMs = 4000;
  const start = Date.now();
  let delay = 40;
  while (Date.now() - start < maxWaitMs) {
    const res = await kvCmd(kvUrl, kvToken, ['SET', PUSH_SUBS_LOCK_KEY, '1', 'NX', 'EX', '15']);
    if (res === 'OK') return true;
    await new Promise(r => setTimeout(r, delay + Math.floor(Math.random() * delay)));
    delay = Math.min(delay * 1.5, 300);
  }
...
export async function releasePushSubsLock(kvUrl, kvToken) {
  try { await kvCmd(kvUrl, kvToken, ['DEL', PUSH_SUBS_LOCK_KEY]); } catch { /* best-effort */ }
}

// Payment-provider webhooks (PayPal, or any future one) can redeliver events
// on transient failures/timeouts — this SET NX EX ensures each event id's
// side effects are ever applied exactly once. 48h TTL comfortably outlives
// any provider's own redelivery window.
export async function claimWebhookEventOnce(kvUrl, kvToken, eventId) {
  const res = await kvCmd(kvUrl, kvToken, ['SET', `webhook_evt:${eventId}`, '1', 'NX', 'EX', '172800']);
  return res === 'OK';
}
// Releases a claim taken above when the handler ultimately couldn't apply
// the event's side effects (e.g. lost a per-salon lock race) — without
// this, the claim being permanent meant a lock-contention failure silently
// dropped the event forever: PayPal only redelivers on a non-2xx response,
// but claimWebhookEventOnce would have told any redelivery "already
// processed" even though nothing was ever actually applied.
export async function unclaimWebhookEvent(kvUrl, kvToken, eventId) {
  try { await kvCmd(kvUrl, kvToken, ['DEL', `webhook_evt:${eventId}`]); } catch { /* best-effort */ }
}

// Guards against a duplicate staff-cancellation notification when two
// concurrent requests cancel the same booking (a double-tap, or two staff
// devices). The normal booking-update branch in api/sync.js now DOES take a
// per-booking lock (acquireBookingLock/releaseBookingLock above) before
// writing, which on its own already prevents two concurrent status changes
// from both landing — but cancelRemovedWorkerBookings (the delete_worker
// action) also cancels bookings directly, on its own separate pass, so two
// independent cancellation paths can still each want to notify the same
// customer about the same booking. This dedupes which one actually does.
export async function claimCancellationNotifyOnce(kvUrl, kvToken, bookingId) {
  const res = await kvCmd(kvUrl, kvToken, ['SET', `cancelnotify:${bookingId}`, '1', 'NX', 'EX', '300']);
  return res === 'OK';
}
...
export async function releaseSlotLock(kvUrl, kvToken, booking) {
  // promoteLock() above PERSISTs this key (removes its TTL) once a booking
  // is confirmed — if this DEL then genuinely fails (a transient KV
  // network blip, not a real conflict) with no retry, the lock has no
  // expiry left to fall back on and stays stuck forever: that exact
  // barber+time becomes permanently unbookable until someone manually
  // clears the key, even though the booking itself is correctly cancelled
  // (overlapsExisting already treats it as free — this only affects the
  // separate exact-slot SET NX gate). One retry after a short delay turns
  // most such blips into a non-issue; only logs (never throws) if both
  // attempts fail, since a cancellation must never be blocked by this.
  try {
    await kvCmd(kvUrl, kvToken, ['DEL', lockKeyFor(booking)]);
  } catch (err) {
    try {
      await new Promise(r => setTimeout(r, 200));
      await kvCmd(kvUrl, kvToken, ['DEL', lockKeyFor(booking)]);
    } catch (err2) {
      console.error('[KV] releaseSlotLock failed after retry — lock may be stuck:', lockKeyFor(booking), err2.message);
    }
  }
}
