...
export async function acquireBarberDayLock(kvUrl, kvToken, salonId, workerId, dateISO) {
  ...
  // Short retry loop, not a queue — booking requests are quick (a fresh
  // bookings re-fetch, one overlap check, one SET NX), so a genuine
  // collision resolves within a second or so. Giving up after that surfaces
  // as a normal "slot taken" conflict to the second caller instead of
  // hanging the request. TTL (10s) is a crash-safety net, not the expected
  // hold time — releaseBarberDayLock always clears it as soon as the
  // critical section finishes; the TTL only matters if that never runs.
