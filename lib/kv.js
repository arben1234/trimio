...
// devices) — the booking-update branch in api/sync.js takes no per-booking
// lock (unlike booking creation's day-lock), so both requests can read the
// same pre-cancellation snapshot and each queue their own notification. The
// booking's cancelled status is written correctly either way; this only
// dedupes which request gets to notify the customer about it.
