...
function renderCustTimes(){
  ...
  $('times').innerHTML=slots.map(t=>`<div class="slot${booked.includes(t)?' busy':''}" data-t="${t}">${t}</div>`).join('');
...
function validateCust(){
  ...
  if(s===1){if(!custData.dateISO)return showErr('cErr','Seleziona un giorno');if(!custData.time)return showErr('cErr','Seleziona un orario');}
...
async function doSubmit(){
  ...
  STATE.bookings.push(bk);await saveState();
...
function doLogin(){
  ...
  if (loginSalonContext) {
    const s = STATE.salons.find(x => x.id === loginSalonContext);
    if (s && usr === s.ownerUsername && pwd === s.ownerPassword) {
...
function showSec(sec){
  ...
  $('newBtn').style.display=(SESSION&&SESSION.role!=='admin'&&(sec==='oggi'||sec==='prossimi'))?'block':'none';
  renderDash();
}
...
function renderDashboardReviews() {
  ...
  html = allReviews.map(rev => {
    const workerLabel = r === 'owner' ? `<span style="font-size:11px; color:#4f46e5; font-weight:700; margin-left:8px; background:#eef2ff; padding:2px 8px; border-radius:10px;">per ${esc(rev.workerName)}</span>` : '';
    return `
      <div class="srv" style="padding:14px; margin-bottom:10px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
          <div>
            <strong style="font-size:14px; color:#111;">${esc(rev.author)}</strong>
            ${workerLabel}
          </div>
          <span style="color:#e5c158; font-weight:700; font-size:12px;">${'★'.repeat(rev.rating)}${'☆'.repeat(5-rev.rating)}</span>
        </div>
        <div style="font-size:13px; color:#444; font-style:italic; line-height:1.4; margin-bottom:6px; padding-left:4px; border-left:2px solid #e5c158;">"${esc(rev.comment)}"</div>
        <div style="font-size:10px; color:#999; text-align:right;">${esc(rev.date || '—')}</div>
      </div>
    `;
  }).join('');
...
function apptCard(b,showActs){
  const canAct = showActs && SESSION && (
    SESSION.role === 'admin' ||
    (SESSION.role === 'owner' && SESSION.salonId === b.salonId) ||
    (SESSION.role === 'barber' && SESSION.workerId === b.workerId)
  );
  const acts=(canAct&&b.status==='confirmed')?`<div class="acard-acts">
  ...
  const phoneRow = b.phone ? `<div class="acard-phone" style="font-size:11.5px; color:#71717a; margin-top:2px;">📞 ${esc(b.phone)}</div>` : '';
  return`<div class="acard ${b.status==='completed'?'completed':b.status==='cancelled'?'cancelled':''}">
    <div class="acard-main">
      ...
      <div class="acard-info">
        <div class="acard-name">${esc(b.name||'Cliente')}${src}</div>
        <div class="acard-svc">${esc(b.service)}</div>${phoneRow}${barberRow}
      </div>
...
function renderClienti(){
  ...
  html+=list.map(c=>{const f=freqTag(c.month);return`<div class="cli">
    ...
    <div class="cli-info"><div class="cli-name">${esc(c.name)} <span class="freq ${f.c}">${f.l}</span></div>
...
function renderAdminStats(){
  ...
  let html=`<div class="kpi-row">
    ${kpiBox(STATE.salons.filter(s=>!s.inactive).length,'Saloni attivi')}
...
async function saveManualAppt(){
  ...
  await saveState();closeModal('modal');
  if(curSec==='oggi'){
    dashDateISO=iso;
    const chip=[...$('oggiDates').querySelectorAll('.chip')].find(c=>c.dataset.iso===iso);
...
async function boot(){
  ...
  const audioBanner = $('audioUnlockBanner');
  if (canStore) { try { if (localStorage.getItem('trimio_audio_unlocked')) { if (audioBanner) audioBanner.style.display = 'none'; } } catch(e){} }
  if (unlockBtn) {
    unlockBtn.addEventListener('click', () => {
      playNotificationSound();
      if (audioBanner) audioBanner.style.display = 'none';
      if (canStore) { try { localStorage.setItem('trimio_audio_unlocked', '1'); } catch(e){} }
    });
  }
  ...
  const checkInitialHash = () => {
    loadSession();
    if (SESSION && SESSION.role) {
      // If URL hash points to a different salon than the session, clear it and show that salon
      if (SESSION.role !== 'admin' && SESSION.salonId) {
        const hInit = (location.hash || '').replace('#', '').toUpperCase().trim();
        if (hInit) {
          const hashSalon = STATE.salons.find(x => x.slug === hInit);
          if (hashSalon && hashSalon.id !== SESSION.salonId) {
            SESSION = {role:null,salonId:null,workerId:null,name:null};
            if (canStore) { try { localStorage.removeItem(SESSION_KEY); } catch(e){} }
            if (hashSalon.inactive) {
              alert(`Spiacenti, il salone "${hashSalon.name}" è temporaneamente inattivo. Contatta l'amministratore.`);
              location.hash = '';
              showView('vLogin');
              return;
            }
            initCustomer(hashSalon);
            showView('vCustomer');
            return;
          }
        }
      ...
      if (SESSION.role !== 'admin' && SESSION.salonId) {
        ...
        if (s && s.inactive) {
          ...
          alert(`Questo salone è stato disattivato dall'amministratore. Accesso negato.`);
          // Continue to normal client routing
        } else {
        ...
        // Admin session - route straight to dashboard!
        showView('vDash');
        initDash();
        if (typeof initPushNotifications === 'function') {
          initPushNotifications();
    ...
    // No hash — password-protect the main page (force login)!
    showView('vLogin');
  };
  checkInitialHash();
