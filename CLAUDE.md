...
Each self-signup salon carries a `billing` object: `declaredWorkerCount` (drives the fee tier — €50 for 1-5 barbers, €100 for 6-10, €150 for 11-15, `feeForWorkerCount()` in `js/app.js`/`api/daily-health-check.js` extrapolates the same +€50-per-5 band beyond that), `paidThroughMonth` ('YYYY-MM', initialized to the signup month so the first partial month is free), `contractSignedAt`/`contractSignedName` (the typed-name e-signature on the in-wizard contract template — not a notarized signature, and the contract text is a placeholder, not reviewed legal language), and `lastWarningEmailSentDate`/`suspendedByBilling` used by the billing cron below. Real bank auto-debit isn't implemented — payment confirmation is a manual admin action (`action: 'mark_salon_paid'` in `api/sync.js`, the 💶 button in the admin Saloni list) until a real processor is wired up.

`api/daily-health-check.js`'s existing daily cron does the billing check too (no new serverless function — Hobby's 12-function cap is already maxed out): for every non-pending salon with a `billing` object, days 2-5 of the month send one warning email/day if unpaid, day 6+ auto-suspends (`inactive: true`) if still unpaid.
